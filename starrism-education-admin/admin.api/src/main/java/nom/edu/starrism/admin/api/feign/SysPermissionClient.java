...
public interface SysPermissionClient {
     ...
     *
     * @param userId userId
     * @return @return {@link Carrier <Set<String>>}
     * @author hedwing
     * @since 2022/11/6
     */
    @GetMapping(value = "/find/url/by/userId")
    Carrier<Set<String>> findPermissionUrlOfUser(@RequestParam(value = "userId") Long userId);
    ...
    Carrier<Set<String>> findPermissionUrlOfRoles(@RequestParam(value = "roles") Collection<Long> roles);
    ...
    Carrier<Set<String>> findPermissionCodeOfUser(@RequestParam(value = "userId") Long userId);
}
